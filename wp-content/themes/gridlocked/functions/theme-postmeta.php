	...
	foreach ($meta_box_image['fields'] as $field) {
		...
		switch ($field['type']) {
			...
			<?php
			  	...
			  	echo '<tr>
			    		<th style="width:25%">
			    			<label for="' . $field['id'] . '">
				    			...
				    			<span style=" display:block; color:#999; margin:5px 0 0 0;">'. $field['desc'].'</span>
			    			</label>
			    		</th>
				<td>';
				...
				echo '<td><input style="float: left;" type="button" class="button" name="', $field['id'], '" id="', $field['id'], '"value="', $meta ? $meta : $field['std'], '" />';
				echo 	'</td>',
			'</tr>';
			}
